    ...
    if (argv["help"]) {
      ...
      try {
        if (input) {
          ...
          if(command.key !="start" && !process.stdin.isTTY && index ==0 ){ actions.push(stdin({encoding:null})) }
        ...
        error("\nSee alligator " + command.key + " --help \n\n" + "Usage: alligator " + command.key + " " + required.join(" ") + " \n\n" + command.key + " error: " + err.message + "\n\n" + data[command.key].desc)
      }
    }
  }
}