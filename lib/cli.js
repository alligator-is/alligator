...
module.exports = function (api, _) {
  return {
    ...
    usage: valid.async(function (command, cb) {
     ...
     if (_.isString(command) && command.indexOf("cli") === 0) return cb(null, Manifest.usage({ command: command, prefix: "cli" }));
      ...
      _(
        Object.keys(capi),
        _.filter(function (i) {
          return endsWith(i, ".usage") && i !== "cli.usage" && i.indexOf('_') == -1 && _.isFunction(capi[i])
        }),
