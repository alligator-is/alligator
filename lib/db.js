...
var Manifest = require("./manifest")("./db.md", __filename)
...
module.exports = function (api,_) {
  ...
  source.path = name
  source.manifest = Manifest.manifest()
  source.usage = valid.async(Manifest.usage,["object?"])
  ...
  function filter(e) {
    return e.doc && e.doc.type === 'peer' && e.doc.lastSeen != null
  }

  source.ls = valid.source(Live(function old(opts) {
    opts = opts || {}
    opts = clone(opts)
    ...
    opts.include_docs =  opts.include_docs || true
    if(opts.include_docs !=true)filter=function(){
      return true
    }
  ...
  source.read = valid.async(function(cb){
    var p = api.swarm._connections[this.id]
    if(!p) return cb()
    if(!p.peer.db) return cb()
    source.load(p.peer.db,cb)
  })
  ...
  _(api.events(), api.events.on({
    closer: function (e) {
      ...
      _(api.swarm._connections[e.id].peer.db.ls({ old: false, live: true, include_docs: true }),ls[e.id],api.swarm.paraMap(function(item,cb){
        setImmediate(function(){
