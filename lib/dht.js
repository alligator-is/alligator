...
var Manifest = require("./manifest")("./dht.md", __filename)
...
module.exports = function (api,_) {
  ...
  timers.bootstrap = asyncInterval(function (next) { 
    if(timers.bootstrap){
      ...
      if(Object.keys(api.swarm._connections).length == 0 )
      return _(connectToClosest(function (err, connection, cb) {
        if (err) return cb()
    ...
    var update = function (cb) {
      ...
      _(
        connectToClosest(function (err, connection, cb) {
          ...
          api.logger.info("bucket", Object.keys(api.swarm._connections).length)
          if (cb) cb()
        }))
    }
  ...
  _(api.swarm.events(),
    api.events.on({
      ready: function (e) {
        addrs = e.address.filter(function(addr){
          return url.parse(addr).protocol.indexOf("+unix") === -1
        })
      ...
      disconnection: function (e) {
        ...
        var found = false
        for (var i in api.swarm._connections) {
          var p = api.swarm._connections[i]
