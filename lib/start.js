...
return Action({
    ...
    run: (opts, cb) => {
      ...
      let plugins = [path.resolve(__dirname+"/../plugins/core/plugin.js"),opts.server==true?path.resolve(__dirname+"/../plugins/server/plugin.js"):path.resolve(__dirname+"/../plugins/client/plugin.js")].concat(api.config.plugins || []).map((plug) => {
      ...
      _(
        ...
        peer.on({
          ready: (e) => {
            api.config.bootstrap.forEach((addr)=>{
              if(api.friends &&  api.friends.put)
              api.friends.put(url.parse(addr).auth,function(){
