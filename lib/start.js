...
return Action({
    ...
    run: (opts, cb) => {
      ...
      let plugins = [path.resolve(__dirname+"/../plugins/dht/plugin.js")].concat(api.config.plugins || []).map((plug) => {
      ...
      if (process.send)
      process.on("message",(msg)=>{ if (msg == 'shutdown') peer.end() })
