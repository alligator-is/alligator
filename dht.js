...
module.exports = function (api) {
  ...
  var replicate = function (cb) {
    ...
    _(connectToClosest(function (err, connection, cb) {
      ...
      api.logger.info('replicating data to peer', connection.peerID)

      api.db.load(connection.peer.db, function (err) {
        api.logger.info('data to', connection.peerID, 'replicated')
        if (cb) cb(err)
      })
  ...
  source.ping = function () {
    return addrs
  }

  source.findNode = function (key) {
